...
class CoreCryptoRSA:
	...
	def hybrid_encrypt(message: str, pk: rsa.RSAPublicKey) -> Any:
		...
		if len(message) <= CryptoConstants.PK_ENC_LEN - CryptoConstants.PK_PAD_LEN:
			...
			p = pk.encrypt(message, padding.OAEP(mgf=padding.MGF1(algorithm=hashes.SHA256()), algorithm=hashes.SHA256(),
			                                     label=None))
			tap_h_data = TapCHData(str(pk), None, str(p), None)
		else:
			k = os.urandom(CryptoConstants.KEY_LEN)
			k_str = b64encode(k).decode('utf-8')
			...
			tap_h_data = TapCHData(nonce.decode('utf-8'), k.decode('utf-8'), p1, p2)
	...
	def hybrid_decrypt(message: str, pk: rsa.RSAPrivateKey) -> str:
		...
		message = Serialize.obj_to_json(message)
		json_inp = Deserialize.json_to_dict(message)
		x = json_inp
	...
	def kdf_tor(message: str) -> dict:
      ...
      """
		...
		kdf_tor_dict = {
			...
			'Kb': str(key[((3 * CryptoConstants.HASH_LEN) + CryptoConstants.KEY_LEN):((3 * CryptoConstants.HASH_LEN) + (2 * CryptoConstants.KEY_LEN))])
		}
